...
1. On the local workstation, clone the Judgels repository (https://github.com/vminfo/olimpier).
...
- `vars.yml` is a file containing environment variables, which will be used in for Ansible playbook [roles](https://github.com/vminfo/olimpier/tree/master/deployment/ansible/roles) during deployment.
...
1. Edit the app version value in `env/vars.yml`:
   ...
   app_version: '2.0'
   ```
   You can get the latest version from https://github.com/vminfo/olimpier/releases. Enter the version without the `v` prefix.
